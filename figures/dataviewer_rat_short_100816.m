...
for scale = 1:2
    for cond = 1:2
        for sub  = 1:length(R.subnames{cond})
            ...
            if scale == 2
                saveallfiguresFIL([R.analysispath R.pipestamp '\results\preprocessing\' FTdata.subject '_short'],'-jpg'); close all
            else
                saveallfiguresFIL([R.analysispath R.pipestamp '\results\preprocessing\' FTdata.subject '_long'],'-jpg'); close all
            end
        end
    end
end
