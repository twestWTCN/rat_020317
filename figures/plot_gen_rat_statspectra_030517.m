function plot_gen_rat_statspectra_030517(R)
close all
cmap = linspecer(3);
...
for feat = 1:length(spctralist) % run through features
    ...
    if strncmp(spctralist{feat},'power',8)
        ...
        undirected = 1; variate = 1; revcheck = 0;
    ...
    elseif strncmp(spctralist{feat},'granger',8)
        ...
        undirected = 0; variate = 2; revcheck = 0;
    elseif strncmp(spctralist{feat},'npd',8)
        spctrname = 'npdspctrm'; dirna = 'NPD';
        undirected = 0; variate = 2; revcheck = 1;
                ...
                elseif undirected == 0 % Group for directional measuremets
                    ...
                    for i = 1:length(R.sourcenames)
                        ...
                        for j = 1:length(R.sourcenames)
                            set2 = find(strncmp([R.sourcenames{j}], FTdata.freqPow.label,size(R.sourcenames{j},2)));
                            for dirc = 1:3
                                fy = squeeze(mean(mean(FTdata.(spctralist{feat})(1).(spctrname){dirc}(set1,set2,:),1),2));
                                fyA{i,j,dirc,cond,sub} = fy;
                                 if revcheck == 1
                                     fyRev{i,j,dirc,cond,sub} = squeeze(mean(mean(FTdata.(spctralist{feat})(2).(spctrname){dirc}(set1,set2,:),1),2));
                                 end
                            end
                        end
                    end
        ...
        if undirected == 1 && variate == 1
            for i = 1:length(R.sourcenames)
                ...
                if statspec == 1
                    ...
                    [ax clustat] = freqclusterplot(OFF,ON,fxA(:,1)',specstat,alpha,[],[],[0 2e-4],cmap)
            ...
            for i = 1:length(R.sourcenames)
                for j = 1:length(R.sourcenames)
                    ...
                    for dirc = 1:3
                        ...
                        if revcheck == 1
                            ON =[fyRev{i,j,dirc,1,:}];
                            OFF = [fyRev{i,j,dirc,2,:}];
                            ax2 = revcheckplot(OFF',ON',fxA{1}',cmap,ax);
                        end
