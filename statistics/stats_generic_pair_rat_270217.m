function stats_generic_pair_rat_270217(R)
...
for cond = 1:2
    for sub  = 1:length(R.subnames{cond})
        ...
        for srcloc = 1:length(R.sourcenames)
            ...
            if sum(strcmpi(statlist, 'COH'))>0
                ...
                if isempty(strmatch(R.sourcenames{srcloc},'fEEG'))
                    for i = 1:length(inds)
                        ...
                        fy =abs(squeeze(FTdata.coherence.cohspctrm(set1,set2,:))); % Combined Coherence spectra
            ...
            %% Imaginary Coherence
            if sum(strcmpi(statlist, 'ICOH'))>0
                if R.clear.genstat == 1 && srcloc == 1
                    FTdata.dirstats.coh = [];
                end
                clear fy
                ...
                if isempty(strmatch(R.sourcenames{srcloc},'fEEG'))
                    for i = 1:length(inds)
                        set1 = inds(i);
                        set2 = find(strncmpi(['fEEG'],FTdata.coherence.label,3)); % STN index
                        fy =imag(squeeze(FTdata.coherence.cohspctrm(set1,set2,:))); % Combined Coherence spectra
                        if size(fy,1)>1 && size(fy,2)>1
                            fy = mean(fy,1);
                        end
                        fxA = FTdata.coherence.freq; % Frequency X vector
                    ...
                    FTdata.dirstats.icoh.(R.sourcenames{srcloc}).banintCoh = intCoh;    % Integrated Coherence
                    FTdata.dirstats.icoh.(R.sourcenames{srcloc}).peakMag = peakMag;  % peak magnitude within band
                    FTdata.dirstats.icoh.(R.sourcenames{srcloc}).peakFrq = peakFrq;  % peak frequency (Hz)
                else
                    FTdata.dirstats.icoh.(R.sourcenames{srcloc}).banintCoh = [];    % Integrated Coherence
                    FTdata.dirstats.icoh.(R.sourcenames{srcloc}).peakMag = [];  % peak magnitude within band
                    FTdata.dirstats.icoh.(R.sourcenames{srcloc}).peakFrq = [];  % peak frequency (Hz)
                end
                clear intCoh peakMag peakFrq
            end
            %% WPLI
            ...
            if sum(strcmpi(statlist, 'DFAPS'))>0
                ...
                clear alpha peb rejrate varfeat ARCoeff
            end
