function stats_generic_pair_rat_050816(R)
    ...
    for cond = 1:2
        for sub  = 1:length(R.subnames{cond})
            ...
            if sum(strcmpi(statlist, 'COH'))>0
                ...
                if isempty(strmatch(R.sourcenames{srcloc},'STN'))
                    for i = 1:length(inds)
                        set1 = inds(i);
                        set2 = find(strncmpi(['STN'],FTdata.coherence.label,3)); % STN index
            ...
            if sum(strcmpi(statlist, 'WPLI'))>0
                ...
                if isempty(strmatch(R.sourcenames{srcloc},'STN'))
                    for i = 1:length(inds)
                        set1 = inds(i);
                        set2 = find(strncmpi(['STN'],FTdata.wpli.label,3)); % STN index
            ...
            if sum(strcmp(statlist, 'MI'))>0
                ...
                for bands = 1:size(R.bbounds,1)
                    for i = 1:length(inds)
                        set1 = inds(i);
                        set2 = find(strncmpi(['STN'],FTdata.label,3));
            ...
            if sum(strcmpi(statlist, 'npd'))>0
                ...
                if isempty(strmatch(R.sourcenames{srcloc},'STN'))
                    for dir = 1:3
                        for i = 1:length(inds)
                            set1 = inds(i);
                            set2 = find(strncmpi(['STN'],FTdata.npd.label,3)); % STN index
