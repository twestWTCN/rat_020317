function convertFT_rat_190717(R)
if R.clear.raw == 1
...
% Highpass filter signal
        ...
        for i = 1:length(R.sourcenames)
           inds = find(strncmp(FTdata.label,R.source_inport_names{i},length(R.source_inport_names{i})));
           for j = 1:length(inds)
               FTdata.label{inds(j),:} = [R.sourcenames{i} FTdata.label{inds(j)}(length(R.source_inport_names{i})+1:end)];
           end
        end
